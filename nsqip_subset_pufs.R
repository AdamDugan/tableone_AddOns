...
## cpt_codes_primary_only: if TRUE, then only consider cases where one of the "cpt_codes" is the primary CPT code (the "CPT" field).
##                         Ignore if cpt_codes = NULL.
## icd10_codes: a character vector of ICD-10 codes to subset the extract to. Ignore when NULL.
...
## icd9_codes: a character vector of ICD-9 codes to subset the extract to (these become rare in more recent years). Ignore when NULL.
...
##                                            "READMUNRELICD95"
## icd_codes_primary_only: if TRUE, then only use the "PODIAG10" and "PODIAG" fields for filtering.

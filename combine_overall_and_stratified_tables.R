...
# dataset = a data.frame object containing, at minimum, all of the variables contained in the table.
# strata.variable = the name of the categorical/pordinal variable that you wish to stratify on. May also be left as NULL for an overall table.
# variables = a vector of the variable names you wish to include as rows in the table.
# variable.labels = an optional vector of labels that will be displayed instead of the variable name, Must be the same length as the variables vector.
# factor.variables = an optional vector of variable names that are numeric but should be coereced into categorical.
# factors_reported.categorical.analyzed.ordinal = an optional vector of variable names where frequencies are reported but the variables are analyzed as ordinal.
...
# descriptor_categorical = a character string of the descriptor to include after categorical variables.
# descriptor_continuous_default = a character string of the descriptor to include after continuous variables.
# descriptor_continuous_nonnormal = a character string of the descriptor to include after non-normal continuous variables.
# digits_pvalue = a numeric value for the number of digits to report for p-values.
# digits_categorical = a numeric value for the number of digits to report for percentages.
# digits_continuous = a numberic value for the number of digits to report for numeric values.
# missing.data_include.as.list = TRUE/FALSE for whether or not to generate a character string to describe the variables with missing data.
# missing.data_include.as.separate.table = TRUE/FALSE for whether or not to generate a separate table summarizing the variables with missing data.
# missing.data_add.to.table = TRUE/FALSE whether or not to add an additional row below a variable for the purposes of reporting missing values.
# missing.data_row.label =  a character string for the row label that will be used for missing data rows
# missing.data_value.prefix = a character string for the pre-fix to include before the missing data frequencies in the body of the table.
